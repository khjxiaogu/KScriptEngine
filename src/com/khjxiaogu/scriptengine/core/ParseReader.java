...
public interface ParseReader {
	 ...
	 * 从指定位置开始读取一定长度的字符串
	 * @param off   the offset<br />位移
	 * @param count read length<br />读取长度
	 * @return return readed <br />
	 *         返回读取后的字符串
	 * @throws KSException Any Engine Exception occurred<br />
	 *                     引擎执行发生的异常。
	 ...
	 * remarks: ch may be ignored if the stream is buffered, and only one char would be rewind.
	 * @param ch the char to insert<br />要被插入的字符
	 */
	...
	/**
	 * Gets the reader's name for logging, which often is the file or script name.<br />
	 * 获取读取器的名字用于填写日志，通常是读取器指向的文件名.
	 *
	 * @return reader's name<br />读取器的名字
	 */
